...
var paths = {
  ...
  dist: {
    ...
    js: 'dist/landing/js',
    libs: 'dist/landing/libs',
    img: 'dist/landing/img',
  },
  src: {
    base: 'src',
    html: 'src/**/*.html',
    css: 'src/landing/css',
    js: 'src/landing/js',
    img: 'src/landing/img/**/*.+(png|jpg|gif|svg)',
    libs: 'src/landing/libs',
...
task('copy:css', function (done) {
  return src([paths.src.css + '/*.css', paths.src.css + '/*.css.map']).pipe(dest(paths.dist.css));
  done();
});
...
task('copy:js', function (done) {
  return src([paths.src.js + '/quick-website.js', paths.src.js + '/quick-website.js.map']).pipe(dest(paths.dist.js));
  done();
});
...
task('copy:html', function (done) {
  return src([paths.src.html, '!' + paths.src.base + '/docs/**/*']).pipe(dest(paths.dist.base));
